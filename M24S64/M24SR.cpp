...
const prog_char AID_NDEF_TAG_APPLICATION2[] PROGMEM = "\xD2\x76\x00\x00\x85\x01\x01";
const prog_char FILE_CC[] PROGMEM = "\xE1\x03";
const prog_char FILE_SYSTEM[] PROGMEM = "\xE1\x01";
const prog_char DEFAULT_PASSWORD[] PROGMEM = "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
...
const prog_char SAMPLE_NDEF_message2[] PROGMEM = "\xD1\x01\x0D\x55\x05\x2B\x33\x35\x38\x39\x31\x32\x33\x34\x35\x36\x37";

// 0x13 txt: "hello, world"
const prog_char SAMPLE_NDEF_message3[] PROGMEM = "\xD1\x01\x0F\x54\x02\x65\x6e\x68\x65\x6c\x6c\x6f\x2c\x20\x77\x6f\x72\x6c\x64";
...
void M24SR::sendApdu_P(uint8_t CLA, uint8_t INS, uint8_t P1, uint8_t P2, uint8_t Lc,prog_char* Data) {
  _data[1] = CLA;
  _data[2] = INS;
  _data[3] = P1;
  _data[4] = P2;
...
void M24SR::writeNdefMessage(NdefMessage* pNDefMsg)
{
    if (pNDefMsg != NULL) {
      ...
      Serial.println(String(rec.getId()));
       selectFile_NDEF_App();
       selectFile_NDEF_file();
       updateBinary_NdefMsgLen0();
